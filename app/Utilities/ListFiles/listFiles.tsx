...
export const FileList: React.FC<FileListProps & {horizontal?: boolean}> = ({
    ...
    const [sortDirection, setSortDirection] =
        useState<string>('/images/asc.png');
                ...
                const unsubscribe = auth.onAuthStateChanged(async user => {
                    if (user) {
                        ...
                        const companyDocRef = doc(
                            db,
                            'Company',
                            'mh3VZ5IrZjubXUCZL381',
                        );
                        const companyDocSnap = await getDoc(companyDocRef);

                        if (companyDocSnap.exists()) {
                            const companyData = companyDocSnap.data();
                            const admins: DocumentReference[] =
                                companyData?.admins || [];
    ...
    const handleDelete = async (fileId?: string) => {
        const deletableFiles = fileId
            ? [fileId]
            : Array.from(selectedFiles).filter(id =>
                  files.find(
                      file =>
                          file.id === id &&
                          (file.uploadedBy === currentUserUid || isAdmin),
                  ),
              );

        const firestorePath: FirestorePath = {
            collectionType:
                collectionPath[2] == 'Buyers'
                    ? 'Buyers'
                    : collectionPath[2] == 'Manufacturers'
                      ? 'Manufacturers'
                      : 'Departments',
            companyId: collectionPath[1],
            departmentId:
                collectionPath[2] == 'Departments'
                    ? collectionPath[3]
                    : undefined,
            buyerId:
                collectionPath[2] == 'Buyers' ? collectionPath[3] : undefined,
            manufacturerId:
                collectionPath[2] == 'Manufacturers'
                    ? collectionPath[3]
                    : undefined,
                        ...
                        !(
                            ...
                            Array.from(selectedFiles).every(id =>
                                files.find(
                                    file =>
                                        file.id === id &&
                                        (file.uploadedBy === currentUserUid ||
                                            isAdmin),
                                ),
                            )
                        )
