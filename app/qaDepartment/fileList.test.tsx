...
const mockProps: FileListProps = {
  collectionPath: ['Company', 'mockCompanyId', 'Departments', 'mockDeptId', 'files'],
...
describe('FileList Component', () => {
  it('renders loading state', () => {
    ...
    expect(screen.getByText('Loading mock department files...')).toBeInTheDocument();
  });
  ...
  it('renders error message when loading fails', async () => {
    ...
    render(<FileList {...mockProps} />);
    expect(await screen.findByText('Failed to load mock department files.')).toBeInTheDocument();
  });

  it('renders no files available message when there are no files', async () => {
    render(<FileList {...mockProps} />);
    expect(await screen.findByText('No files available.')).toBeInTheDocument();
  });

  it('renders a list of files when data is fetched', async () => {
    const mockFiles = [
      { id: '1', fileName: 'File1.pdf', download: 'mock-url-1', filePath: 'path1' },
      { id: '2', fileName: 'File2.docx', download: 'mock-url-2', filePath: 'path2' },
    ];
