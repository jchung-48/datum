...
describe('Home Page', () => {
  ...
  test('redirects authenticated user to department URL', async () => {
    ...
    const mockUser = { getIdTokenResult: jest.fn().mockResolvedValue({ claims: { companyId: 'company123' } }) };
    (auth.onAuthStateChanged as jest.Mock).mockImplementation((callback) => callback(mockUser));

    // Mock Firestore document calls
    const mockDepartmentRef = { id: 'dep123' };
    const mockEmployeeSnap = { exists: () => true, get: () => [mockDepartmentRef] };
    ...
    render(<Home />);
    await waitFor(() => expect(mockPush).toHaveBeenCalledWith('/department-url'));
  });
  ...
  test('redirects unauthenticated user to /workplaces', async () => {
    (auth.onAuthStateChanged as jest.Mock).mockImplementation((callback) => callback(null)); // No user (unauthenticated)

    render(<Home />);
    await waitFor(() => expect(mockPush).toHaveBeenCalledWith('/workplaces'));
  });
});
