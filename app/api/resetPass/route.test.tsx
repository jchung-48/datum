...
describe('resetPass API route', () => {
  ...
  it('should return 200 and send reset email on valid input', async () => {
    ...
    (admin.auth().generatePasswordResetLink as jest.Mock).mockResolvedValue(mockResetLink);
    ...
    expect(admin.auth().generatePasswordResetLink).toHaveBeenCalledWith(mockEmail);
    expect(sendResetEmail).toHaveBeenCalledWith(mockEmail, mockResetLink, false);
  });

  it('should return 500 if there is an error generating the reset link', async () => {
    const mockEmail = 'test@example.com';
    (admin.auth().generatePasswordResetLink as jest.Mock).mockRejectedValue(new Error('Invalid email'));
    ...
    const body = await res.json();
    expect(body.message).toContain('Error reseting password: Invalid email');
    expect(admin.auth().generatePasswordResetLink).toHaveBeenCalledWith(mockEmail);
  });
});
