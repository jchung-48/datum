...
describe('sendResetEmail', () => {
  ...
  it('should send a "Set Your Datum Password" email for first-time setup', async () => {
    ...
    expect(mockSendMail).toHaveBeenCalledWith({
      ...
      html: expect.stringContaining('Your Datum account has been created'),
    });
  });
  ...
  it('should handle errors gracefully', async () => {
    ...
    await expect(sendResetEmail(email, resetLink, firstTime)).resolves.not.toThrow();

    expect(console.error).toHaveBeenCalledWith('Error sending reset link:', expect.any(Error));
  });
});
