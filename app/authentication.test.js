...
describe('Authentication library', () => {
    ...
    describe('getDepartments', () => {
        ...
        it('should throw an error if fetching departments fails', async () => {
            ...
            await expect(getDepartments('testCompany')).rejects.toThrow(
                'Firestore error',
            );
        });
    });
    ...
    describe('signInUser', () => {
        ...
        it('should throw an error if no user document is found', async () => {
            ...
            await expect(
                signInUser('test@example.com', 'password123', 'testCompany'),
            ).rejects.toThrow('No user found for the given company.');
        });
    });

    describe('sendVerificationCode', () => {
        it('should send a verification code to the phone number', async () => {
            PhoneAuthProvider.verifyPhoneNumber.mockResolvedValue(
                'testVerificationId',
            );
            ...
            expect(PhoneAuthProvider.verifyPhoneNumber).toHaveBeenCalledWith(
                phoneNumber,
                expect.any(Object),
            );
    ...
    describe('getEmployeeProfile', () => {
        ...
        it('should throw an error if employee does not exist', async () => {
            ...
            await expect(getEmployeeProfile('testUid')).rejects.toThrow(
